...
const HEADER = 'Manage Patients';
const BREADCRUMB = 'patients';
const KEYWORDS = '';
const PAGE_DESC = 'the admin management page';

$pgURL = $patientRoot . "patients";

#logics for this page
include '../inc/logics/patients.php';
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="bg-white rounded py-3 px-2">
                    <!-- <a href="<?= $patientRoot ?>patients?min=true&do=create" class="btn btn-sm btn-outline-info rounded-circle"><i class="bi bi-plus"></i></a> -->
                    <a href="<?= $patientRoot ?>patients" class="btn btn-sm btn-outline-info rounded-circle float-end"><i class="bi bi-bootstrap-reboot"></i></a>
                </div>
            </div>
        </div>
                ...
                <div class="col-md-4">
                    <div class="card">
                        <div class="card-header">
                            <div class="">
                                <h4 class="card-title d-inline-block"><?= (isset($cardTitle)) ? $cardTitle : ''; ?></h4>
                                <a href="<?= $patientRoot ?>patients" class="float-end text-danger fw-bolder"><i class="bi bi-x fw-bold"></i></a>
                            </div>
            ...
            <div class="col-md-<?php if (isset($_GET['min']) && $_GET['min'] == 'true') {
                ...
                <div class="card">
                                                ...
                                                <div class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                        <?php if ($row['status'] == 'active'): ?>
                                                            <a class="dropdown-item" href="<?= $patientRoot ?>patients?id=<?= $row['id'] ?>&min=true&do=edit">Edit</a>
                                                            <a class="dropdown-item" href="<?= $patientRoot ?>patients?id=<?= $row['id'] ?>&do=reset-pwd">Reset Password</a>
                                                            <a class="dropdown-item" href="<?= $patientRoot ?>patients?id=<?= $row['id'] ?>&do=deactivate">Deactivate</a>
                                                        <?php else: ?>
                                                            <a class="dropdown-item" href="<?= $patientRoot ?>patients?id=<?= $row['id'] ?>&do=activate">Activate</a>
                                                            <a class="dropdown-item" href="<?= $patientRoot ?>patients?id=<?= $row['id'] ?>&do=delete">Delete</a>
                                                        <?php endif ?>
                                                    </div>
                                                </div>
                                            </div>
                    ...
                    <div class="card-footer bg-transparent">
                        <a href="<?= $patientRoot ?>patients?do=truncate" class="btn btn-sm btn-outline-warning">Truncate Patients</a>
                    </div>
                </div>
            </div>
        </div>
