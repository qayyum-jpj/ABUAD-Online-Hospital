...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        #header h4 { color: #0f9b8e; font-size: 1rem; }
        #header span { font-size: 0.85rem; color: #aaa; }
        #videos { flex: 1; display: flex; gap: 10px; padding: 15px; flex-wrap: wrap; align-items: center; justify-content: center; }
        video { background: #000; border-radius: 10px; width: 45%; max-height: 45vh; object-fit: cover; }
        #localVideo { border: 2px solid #0f9b8e; }
        #remoteVideo { border: 2px solid #444; }
        #controls { background: #16213e; padding: 15px; display: flex; justify-content: center; gap: 15px; }
        .ctrl-btn { border: none; border-radius: 50%; width: 50px; height: 50px; font-size: 1.2rem; cursor: pointer; transition: 0.2s; }
        #btnMute   { background: #0f9b8e; color: #fff; }
        #btnCam    { background: #0f9b8e; color: #fff; }
        #btnHangup { background: #e74c3c; color: #fff; }
        .ctrl-btn:hover { opacity: 0.8; transform: scale(1.05); }
        #status { text-align: center; padding: 8px; font-size: 0.85rem; color: #aaa; background: #0d0d1a; }
        #waitMsg { position: absolute; top: 50%; left: 50%; transform: translate(-50%,-50%); text-align: center; color: #aaa; display: none; }
        #waitMsg i { font-size: 3rem; display: block; margin-bottom: 10px; }
    </style>
</head>
<body>

<div id="header">
    <h4>🏥 ABUAD Online Hospital — Meeting Room</h4>
...
async function createPeer(isInitiator) {
    ...
    peerConnection.onicecandidate = (e) => {
        if(e.candidate) ws.send(JSON.stringify({ type: 'ice', candidate: e.candidate, room: roomId }));
    };
