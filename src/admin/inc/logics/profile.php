...
if(isset($_POST['updateInfo'])){
    ...
    if($q == 'success'){
        ...
        if($doLog == 'logged'){
            ...
            $url = $adminRoot.'profile?vw=e-info';
...
if(isset($_POST['updatePassword'])){
    ...
    # validate inputs
    if(empty($password)){ $errs[] = $passwordError = "cannot be empty"; }else{
    ...
    if(empty($newPassword)){ $errs[] = $newPasswordError = "cannot be empty"; }
    if(empty($newPassword2)){ $errs[] = $newPassword2Error = "cannot be empty"; }
    ...
    if(count($errs) == 0){
        ...
        if($q == 'success'){
            ...
            if(move_uploaded_file($_FILES['userImage']['tmp_name'], $uploadPath.$newFileName)){
                ...
                $url = $adminRoot.'profile?vw=e-image';
...
if(isset($_GET['do'])){
    ...
    if($do == 'reset-image'){
        ...
        $pgURL = $adminRoot."profile?vw=e-info";
        $prompt = true;
